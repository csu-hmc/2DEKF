function EKF5good(filename, mode)%We got better result by making P smaller
	...
	model.dt = 0.00016;		% time step for the Kalman filter

	% covariances (these need more thinking to get good values!!)
% 	Qu = sparse(0.00001*eye(model.Nmus));			% covariance in unknown muscle controls Decreasing Qu makes the results worst
% 	Qz = sparse(diag([ zeros(1,9) 1 1 1 zeros(1,38)]));		% process noise covariance
...
%Goodresult 1
%       Qu = sparse(0.00001*eye(model.Nmus));			% covariance in unknown muscle controls Decreasing Qu makes the results worst
% 	Qz = sparse(0.01*eye(model.Nstates));		% process noise covariance
% 	R  = sparse(0.0000001*eye(model.Nmeas));			% measurement noise covariance

%GoodResult 2
% Qu = sparse(0.0000001*eye(model.Nmus));			% covariance in unknown muscle controls Decreasing Qu makes the results worst
% 	Qz = sparse(0.01*eye(model.Nstates));		% process noise covariance
% 	R  = sparse(0.0000001*eye(model.Nmeas));			% measurement noise covariance
	...
	P =0.00001*eye(model.Nstates); 	% estimation error covariance
	...
	for k = 1:N-1
		...
		P = (eye(model.Nstates) - K * H) * P ;  % estimation error covariance measurement update     Eq 13.49(3)
